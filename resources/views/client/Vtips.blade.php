...
<head>
    ...
    <title>Tips And Trick Content</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-300">
    <nav class="bg-gray-800 w-full shadow-lg flex justify-between">
        <h2 class="text-white font-semibold text-xl p-5">Tips Content</h2>
    ...
    <div class="container mx-auto py-10">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-slate-200 rounded-lg shadow-md overflow-hidden ml-10">
                <img class="w-full object-cover" src="{{ $tips->gambar_tips }}" alt="{{ $tips->nama_tips }}">
                <div class="p-5">
                    <h3 class="text-lg font-semibold">{{ $tips->nama_tips }}</h3>
                    <p class="text-gray-600 mt-2 overflow-hidden text-ellipsis whitespace-normal">
                        {{ Str::limit($tips->deskripsi_tips, 100) }}
                        <span class="text-indigo-500 hover:text-indigo-700 cursor-pointer" onclick="showMore(this)">Read more</span>
                        <span class="hidden" id="more-text">{{ $tips->deskripsi_tips }} <span id="sembunyikan" class="text-indigo-500 hover:text-indigo-700 duration-200 cursor-pointer" onclick="hideMore(this)"> Hide</span></span>
                    </p>
                    <a href="{{ $tips->video_tips }}" target="_blank" class="text-indigo-500 hover:text-indigo-700 mt-4 duration-200 inline-block">Lihat video tutorial</a>
                </div>
            </div>
            @empty
